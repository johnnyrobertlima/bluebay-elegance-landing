...
export const createItemDetailsMap = (stockItems: any[]) => {
  return stockItems.reduce((map, item) => {
    const itemCode = (item["ITEM_CODIGO"] || item["item_codigo"])?.toString().trim();
    if (itemCode && !map.has(itemCode)) {
      // Prioritize GRU_DESCRICAO but fallback to GRU_CODIGO as requested by USER
      const groupValue = item["GRU_DESCRICAO"] || item["gru_descricao"] || item["GRU_CODIGO"] || item["gru_codigo"] || "";

      map.set(itemCode, {
        DESCRICAO: item["DESCRICAO"] || item["descricao"] || "",
        GRU_DESCRICAO: groupValue,
...
export const mapStockItems = (
  ...
  return stockItems.reduce((acc, item) => {
    ...
    const fisico = Number(item["FISICO"] || item["fisico"]) || 0;
    ...
    const dataCadastro = itemDetail["DATACADASTRO"] || itemDetail["datacadastro"] || item["DATACADASTRO"] || item["datacadastro"];
