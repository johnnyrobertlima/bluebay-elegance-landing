...
export const groupSalesByItem = (salesData: any[]) => {
  return salesData.reduce((acc, sale) => {
    ...
    const quantidade = Number(sale["QTDE_PEDIDA"] || sale["QUANTIDADE"]) || 0;
    ...
    const saleDateStr = sale["DATA_PEDIDO"] || sale["DATA_EMISSAO"];
    if (saleDateStr) {
      ...
      if (!acc[itemCode].DATA_ULTIMA_VENDA || saleDate > new Date(acc[itemCode].DATA_ULTIMA_VENDA)) {
        acc[itemCode].DATA_ULTIMA_VENDA = saleDateStr;
      }
    }
