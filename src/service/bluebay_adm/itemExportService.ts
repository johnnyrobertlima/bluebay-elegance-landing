...
export const importItemsFromExcel = async (file: File): Promise<{
  ...
  try {
    ...
    if (data.length > 0) {
      console.log("DEBUG IMPORT - Exemplo de chaves do primeiro item:", Object.keys(data[0]));
      console.log("DEBUG IMPORT - Exemplo do primeiro item:", data[0]);
    }
      ...
      for (const item of batch) {
        try {
          ...
          const updateData: any = {
            DESCRICAO: item.Descrição || item['Descrição'] || item.Descricao || item['Descricao'] || item.Descriçao || item['Descriçao'] || item.Description || item.Produto || item.Nome || item.Name || undefined,
            CODIGOAUX: item['Código Auxiliar'] || item['Codigo Auxiliar'] || item.CODIGOAUX || undefined,
            GRU_CODIGO: item['Código do Grupo'] || item['Codigo do Grupo'] || item.GRU_CODIGO || undefined,
            GRU_DESCRICAO: item['Descrição do Grupo'] || item['Descricao do Grupo'] || item.GRU_DESCRICAO || undefined,
            empresa: item.Empresa || item.empresa || undefined,
            estacao: item.Estação || item.Estacao || item.estacao || undefined,
            genero: item.Gênero || item.Genero || item.genero || undefined,
            faixa_etaria: item['Faixa Etária'] || item['Faixa Etaria'] || item.faixa_etaria || undefined,
