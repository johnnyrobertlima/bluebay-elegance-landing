-- Migration: Add Representative Analysis Features (Potencial Perdido & Orders List)
-- Date: 2026-01-29 09:00:00

-- 1. Create RPC for Representative Orders List (Aggregated by Order)
...
BEGIN
  ...
  SELECT 
    p."PED_NUMPEDIDO",
    MAX(p."PEDIDO_OUTRO") as "PEDIDO_OUTRO", -- Max/First as it should be same for the order
    MIN(p."DATA_PEDIDO") as "DATA_PEDIDO",
    MAX(p."STATUS") as "STATUS", -- Usually same for all items in order
  ...
  WHERE p."REPRESENTANTE" = p_rep_id
    ...
    AND p."STATUS" != '4' -- Cancelled usually excluded from lists, or should we include for history? User said "4 - Cancelado" in translation, implying they want to see them.
    -- Wait, user asked to LIST data and provided translation for 4. So I should probably INCLUDE 4.
    -- Re-reading prompt: "Status do pedidos usuar o campo STATUS traduzindo os dados: ... 4 - Cancelado". 
    -- So I must NOT filter out status 4 here if I want to show them.
    -- However, standard dashboard usually filters. 
    -- Let's include everything in the list for analysis.
    -- But excluding from the WHERE clause I previously wrote.
...
-- 2. Update get_commercial_dashboard_stats_v3 to include 'totalPotencialPerdido'
-- Copying previous logic and adding the new field.
...
BEGIN
  ...
  -- Removed STATUS != 4 filter globally here? No, 'total' stats usually exclude cancelled.
  -- But 'Potencial Perdido' specifically asks for STATUS=3.
  -- Let's keep excluding 4 for general stats, but careful if we needed 4 for something else.
  -- The user request for Potencial Perdido is strictly STATUS=3.
  -- The standard Dashboard usually filters out Cancelled (4).
  DELETE FROM tmp_ped_base_v3 WHERE "STATUS" = '4';
  ...
  total_ped_metrics AS (
      SELECT 
        ...
        COUNT(DISTINCT "PED_NUMPEDIDO") as ped_count,
        -- NEW METRIC: Potencial Perdido (Status 3 Only)
        COALESCE(SUM(CASE WHEN "STATUS" = '3' THEN ("QTDE_SALDO" * "VALOR_UNITARIO") ELSE 0 END), 0) as potencial_perdido
      FROM tmp_ped_base_v3
  ),
  -- Daily Final
  final_daily AS (
  ...
  SELECT json_build_object(
    ...
    'totals', (
      SELECT json_build_object(
        ...
        'totalPedidosCount', (SELECT ped_count FROM total_ped_metrics),
        'totalPotencialPerdido', (SELECT potencial_perdido FROM total_ped_metrics) -- Added Field
      )
    ),
    'costCenters', (
      SELECT json_agg(json_build_object(
