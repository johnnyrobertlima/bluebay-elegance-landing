-- Migration: Update Dashboard Stats RPC to include Potencial Perdido (including Cancelled)
-- Date: 2026-01-29 10:15:00
...
BEGIN
  ...
  -- Removed strict DELETE of Status 4 so we can use it for Potencial Perdido calc.
  -- We will filter it out of other stats manually.
  ...
  -- Global Totals (Complex Filter for Potencial Perdido)
  total_metrics AS (
      SELECT 
  ...
  total_ped_metrics AS (
      SELECT 
        -- Standard Totals (Exclude Cancelled)
        COALESCE(SUM(CASE WHEN "STATUS" != '4' THEN valor_total ELSE 0 END), 0) as ped_val,
        COALESCE(SUM(CASE WHEN "STATUS" != '4' THEN quantidade ELSE 0 END), 0) as ped_qtd,
        COUNT(DISTINCT CASE WHEN "STATUS" != '4' THEN "PED_NUMPEDIDO" END) as ped_count,
        
        -- NEW METRIC: Potencial Perdido (Status 3 + 4)
        COALESCE(SUM(CASE 
            WHEN "STATUS" IN ('3', '4') THEN ("QTDE_SALDO" * "VALOR_UNITARIO") 
            ELSE 0 
        END), 0) as potencial_perdido
      FROM tmp_ped_base_v3
  ),
  -- Daily Final
  final_daily AS (
  ...
  SELECT json_build_object(
    ...
    'totals', (
      SELECT json_build_object(
        ...
        'totalPotencialPerdido', (SELECT potencial_perdido FROM total_ped_metrics)
      )
    ),
    'costCenters', (
      SELECT json_agg(json_build_object(
