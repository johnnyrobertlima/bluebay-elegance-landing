-- Migration: Add totalPedidosCount to Dashboard RPC
-- Date: 2026-01-28 17:50:00
...
BEGIN
  ...
  total_ped_metrics AS (
      SELECT 
        ...
        COUNT(DISTINCT "PED_NUMPEDIDO") as ped_count  -- ADDED: Distinct Order Count
      FROM tmp_ped_base_v3
  ),
  -- Daily Final
  final_daily AS (
  ...
  SELECT json_build_object(
    ...
    'totals', (
      SELECT json_build_object(
        ...
        'totalPedidosCount', (SELECT ped_count FROM total_ped_metrics) -- ADDED: New field
      )
    ),
    'costCenters', (
      SELECT json_agg(json_build_object(
