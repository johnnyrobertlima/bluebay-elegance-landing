...
DECLARE
  v_days_diff INTEGER;
BEGIN
  -- Calculate days in period for coverage calculations
  ...
  RETURN QUERY
  WITH base_items AS (
    -- Main item filtering done on BLUEBAY_ITEM as requested
    SELECT 
      i."ITEM_CODIGO",
      i."DESCRICAO",
      i."GRU_CODIGO",
    ...
    WHERE (p_search_term IS NULL OR p_search_term = '' 
           OR i."ITEM_CODIGO" ILIKE '%' || p_search_term || '%' 
           OR i."DESCRICAO" ILIKE '%' || p_search_term || '%')
  ...
  vendas AS (
    -- Using BLUEBAY_PEDIDO for sales data as requested
    SELECT 
    ...
    WHERE (p_start_date IS NULL OR p."DATA_PEDIDO" >= p_start_date)
      ...
      AND p."STATUS" != '4' -- Exclude cancelled
  ...
  SELECT 
    ri.item_codigo::TEXT as item_codigo, 
    ri.descricao::TEXT as descricao, 
    ri.gru_descricao::TEXT as gru_descricao, 
    ri.datacadastro::DATE as datacadastro,
    ri.fisico::NUMERIC as fisico,
    ri.disponivel::NUMERIC as disponivel,
    ri.reservado::NUMERIC as reservado,
    ri.entrou::NUMERIC as entrou,
    ri.limite::NUMERIC as limite,
    ri.qtd_vendida::NUMERIC as qtd_vendida,
    ri.valor_total_vendido::NUMERIC as valor_total_vendido,
    ri.preco_medio::NUMERIC as preco_medio,
    0::NUMERIC as custo_medio,
    ri.data_ultima_venda::DATE as data_ultima_venda,
    ri.giro_estoque::NUMERIC as giro_estoque,
    ri.percentual_estoque_vendido::NUMERIC as percentual_estoque_vendido,
    ri.dias_cobertura::INTEGER as dias_cobertura,
    ri.produto_novo::BOOLEAN as produto_novo,
    ri.ranking::BIGINT as ranking,
    ri.total_count::BIGINT as total_count
  FROM ranked_items ri
  ORDER BY ri.ranking ASC
  LIMIT p_limit
  OFFSET p_offset;
END;
$$;
