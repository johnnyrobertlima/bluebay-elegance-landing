-- Migration: Fix NULL/Zero Valor Unitario in MV and Re-apply RPC
-- Date: 2026-01-28 17:00:00

-- 1. Fix Data in MV_BLUEBAY_FATURAMENTO_CENTRO_CUSTO
-- We suspect existing rows have NULL or 0 valor_unitario, causing 0 Total Faturado.
DO $$
BEGIN
    RAISE NOTICE 'Updating MV_BLUEBAY_FATURAMENTO_CENTRO_CUSTO with data from BLUEBAY_FATURAMENTO...';
    
    UPDATE public."MV_BLUEBAY_FATURAMENTO_CENTRO_CUSTO" t
    SET 
        valor_unitario = s."VALOR_UNITARIO",
        quantidade = s."QUANTIDADE",
        valor_nota = CASE WHEN s."VALOR_NOTA" > 0 THEN s."VALOR_NOTA" ELSE (s."QUANTIDADE" * s."VALOR_UNITARIO") END -- Optional fix for valor_nota if needed elsewhere
    FROM public."BLUEBAY_FATURAMENTO" s
    WHERE t."ID_EF_DOCFISCAL"::text = s."ID_EF_DOCFISCAL"::text 
      AND t."ID_EF_DOCFISCAL_ITEM"::text = s."ID_EF_DOCFISCAL_ITEM"::text
      AND (t.valor_unitario IS NULL OR t.valor_unitario = 0);
      
    RAISE NOTICE 'Update complete.';
END $$;

-- 2. Re-apply RPC to Ensure Filters are correct and Calculation uses valor_unitario
...
BEGIN
  ...
  -- Fix: Calculate Total as (Quantidade * Valor_Unitario)
  ...
  -- FALLBACK STRATEGY: Update Representative from Orders if missing in Invoice
  ...
  -- 4. Aggregations (Using valor_total_calc for invoices)
