-- Migration: Create Dashboard RPC V3 to fix ambiguity and correct filters
...
BEGIN
  -- 1. Invoices Base
  -- Fix: Source from MV_BLUEBAY_FATURAMENTO_CENTRO_CUSTO as requested by user.
  -- Note: MV columns are lowercase.
  ...
  -- Filtra transações desativadas (if not already handled in MV population, but good for safety)
  ...
  -- This fixes the issue where "Faturamento" is 0 because the direct link failed.
  ...
  -- 3. Apply Filters with IS DISTINCT FROM to correctly handle NULLs
  ...
  IF p_representante IS NOT NULL AND p_representante != 'none' THEN
    ...
    ELSE
      -- "IS DISTINCT FROM" removes non-matches AND nulls (unless null is what we want)
